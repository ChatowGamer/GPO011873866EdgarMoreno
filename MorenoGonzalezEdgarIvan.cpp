...
// https://github.com/ChatowGamer/GPO011873866EdgarMoreno //


int* numart, * year, registros, n = 1;
string* name, * clasf, * caract, * descp, * gnr;
...
int main()
{
	setlocale(LC_ALL, "spanish"); //Programa corriendo en español
	SetConsoleCP(1252);
	...
	case 2: //cambiar articulo
		modificar();
		system("pause");
		return main();
		break;

	case 3: //suprimir Articulo
		eliminar();
		system("pause");
		return main();
		break;

	case 4:  // Todos los objetos
		lista();
		system("pause");
		return main();
		break;
...
void alta()
{
	...
	if (registros > 0)
	{
		...
		for (int i = 0; i < registros; i++)
		{
			...
			printf("---------------------------------------------------- \n\n");  //Puse separadores para que se vea mejor
...
void archivo()
{
	...
	for (int i = 0; i < registros; i++)
	{
		if (numart[i] != 0)
		{
			...
			archivo << "Nombre: " << name[i].c_str() << endl;
